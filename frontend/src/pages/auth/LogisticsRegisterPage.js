...
const LogisticsRegisterPageInner = () => {
  ...
  const onFinish = async (values) => {
    ...
    if (!values.email.endsWith('@closetrush.com')) {
      message.error('Logistics partners must register with a @closetrush.com email address.');
      return;
    }
          ...
          <div className="w-full max-w-md text-center">
            <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-10">
              <div className="w-20 h-20 rounded-full bg-green-500/20 border-2 border-green-500 flex items-center justify-center mx-auto mb-6">
                <ClockCircleOutlined className="text-green-400 text-4xl" />
              </div>
              <Title level={2} className="!text-white !mb-3">Registration Submitted!</Title>
              <Paragraph className="text-slate-300 text-base mb-8">
                Your logistics partner account has been created and is now <strong className="text-green-400">pending admin approval</strong>.
              ...
              <Steps
                ...
                items={[
                  ...
                  {
                    title: <span className="text-green-400 font-semibold">Awaiting Admin Approval</span>,
                    description: <span className="text-slate-400">Admin will review your profile</span>,
                    status: 'process',
                    icon: <ClockCircleOutlined className="text-green-400" />
                  },
                  {
                    title: <span className="text-slate-400 font-semibold">Access Dashboard</span>,
                    description: <span className="text-slate-500">Login and start delivering orders</span>,
                    status: 'wait',
                  }
                ]}
              />
              ...
              <Button
                ...
                className="h-12 font-semibold rounded-xl border-2 border-green-500 text-green-400 hover:bg-green-500/10 bg-transparent"
              >
                Go to Logistics Login
              </Button>
            </div>
          </div>
        </Content>
        ...
        <div className="w-full max-w-lg">
          ...
          <div className="text-center mb-8">
            ...
            <Paragraph className="text-slate-400 text-base">
              Register your ClosetRush logistics partner account
            </Paragraph>
          </div>
          ...
          <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-8">
            <Form onFinish={onFinish} layout="vertical">
              ...
              >
                <Input
                  prefix={<MobileOutlined className="text-green-400" />}
...
const LogisticsRegisterPage = () => (
  <App>
    <LogisticsRegisterPageInner />
  </App>
);

export default LogisticsRegisterPage;
